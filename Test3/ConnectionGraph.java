...
public class ConnectionGraph {
    ...
    public void addEdge(String username1, String username2) {
    ...
    /* 
    public String showSecondDegreeConnections(String username, String targetUsername) {
    StringBuilder sb = new StringBuilder();
    Vertex vertex = getVertex(username);
    Vertex targetVertex = getVertex(targetUsername);

    if (vertex != null && targetVertex != null) {
        LinkedList<Vertex> firstDegreeNeighbors = vertex.getNeighbors();

        for (Vertex firstDegreeNeighbor : firstDegreeNeighbors) {
            if (firstDegreeNeighbor.equals(targetVertex)) {
                LinkedList<Vertex> secondDegreeNeighbors = firstDegreeNeighbor.getNeighbors();
                for (Vertex secondDegreeNeighbor : secondDegreeNeighbors) {
                    if (!secondDegreeNeighbor.equals(vertex) && !firstDegreeNeighbors.contains(secondDegreeNeighbor)) {
                        sb.append(secondDegreeNeighbor.getUsername()).append(",");
                ...
                break; // No need to check further first degree neighbors
            }
        }
    } else {
        sb.append("Vertex ").append(username).append(" or Vertex ").append(targetUsername).append(" not found.");
    }
    ...
    */
    ...
    public void loadAdjacency() {
        index = 0; // Reset the index counter before loading the adjacency
